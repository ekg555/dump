...
Logfile <- paste0("coord2rsid.r.LOG_", Sys.Date(), ".txt")

# ===========================================================================================
# Annot.dir <- 'R:/StaffFolders/KonagayaEugene/Annotations/RMS' # error UCSC site down?
# Annot.dir <- 'R:/StaffFolders/KonagayaEugene/Annotations' # for CCLS office
...
coord2rsid <- function (parent.dir = Annot.dir) {
  ...
  for (i in 1:length(folders)) {
    ...
    if (!length(files) > 0) {
      ...
      for (j in 1:length(files)) {
        ...
        # #    FOR NOW... HOLD OFF ON INCORPORATING 'rsid2annot.r' (more like rsid2UCSCvai.r)
        ...
        print(ls())
        # rm(list=ls()[!(ls() %in% 
        #                  c( 'coord2rsid', 'hg19', 'i', 'j', 'parent.dir') )])
